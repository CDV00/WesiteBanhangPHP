		...
		<?php foreach ($products as $product) { ?>
			<div class="col-md-3 col-sm-6 text-center card">
				...
				<div class="card-body alert alert-success">
					...
					<span class="text-decoration-line-through text-secondary d-inline-block text-truncate" style="font-size: 110%;"><?php echo number_format($product['salePrice']); ?>đ</span><span class="text-dark d-inline-block text-truncate" style="font-size: 110%; margin-left:5%;"> <?php echo number_format($product['Price']); ?>đ</span>
					<a class="btn btn-primary" href="<?php echo BASE_URL ?>cart/add/<?php echo $product['productId'] ?>/<?php echo $product['productName'] ?>/<?php if ($product['salePrice'] <> 0) echo $product['salePrice'];
																																							else echo $product['Price']; ?>/<?php echo $product['Image'] ?>">Add to Cart</a>
				</div>
